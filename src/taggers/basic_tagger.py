...
class BasicTagger(object):
    ...
    def evaluate(self, gold_file, test_file,eval_out_file=None):
        gold_df = ld.load_gold_train(gold_file)
        test_tagged_df = ld.load_gold_train(test_file)
        if eval_out_file is None:
            eval_out_file=os.getcwd() + '\\basic_tagger.eval'

        eval.output_eval(eval_out_file, model_name="baseline", test_file=test_file,
                         gold_file=gold_file,
                         gold_df=gold_df, test_tagged_df=test_tagged_df)
        print "output basic_tagger evaluation file at: {}".format(eval_out_file)
