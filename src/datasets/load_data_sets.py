...
def load_data(segments_file, is_tagged=False):
    ...
    seg_df = pd.read_csv(segments_file, sep='\t', header=None, skip_blank_lines=False, keep_default_na=False,
                         na_values=[''])
    ...
    seg_df['EMPTY_LINE'] = seg_df.SEG.isnull()

    for index, row in seg_df.iterrows():
        if not row['EMPTY_LINE']:
            ...
            word_num = 1
            sen_num = sen_num + 1
    seg_df = seg_df[seg_df['EMPTY_LINE'] == False]
    return seg_df.drop('EMPTY_LINE', 1)
...
def split_to_fold_files(segments_file, num_folds):
    ...
    if not os.path.exists(folds_dir + "\\training_set_folds"):
        os.makedirs(folds_dir + "\\training_set_folds")

    folds_dir = folds_dir + "\\training_set_folds"
...
def train_tagged_from_folds(folds_paths_df, min_foldid_train, max_foldid_train, out_dir=None):
    if out_dir is None:
        ...
        out_dir = folds_dir + "\\training_files"
